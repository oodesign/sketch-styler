...
window.DrawElements2 = (byArtb, totalLayers, layersWithNoMatches) => {
  globalUnstyledLayers = byArtb;
  globalTotalLayers = totalLayers;
  globalAssignLayers = 0;
  var inner = "";
  ...
  for (var artboard in byArtb) {
    ...
    var unstyledTextLayers = byArtb[artboard];
    if (layerID == 0)
      frag.appendChild(createTextWithClass(byArtb[artboard][0].artboardName, 'primaryBigText'));
    else
      frag.appendChild(createTextWithClass(byArtb[artboard][0].artboardName, 'primaryBigText separeHeader'));
